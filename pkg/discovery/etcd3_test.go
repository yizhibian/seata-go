		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(put.putMap) != 0 {
				for key, value := range put.putMap {
					...
					defer cancel()
					_, err = cli.Put(ctx, tt.args.etcdPrefix+key, value)
					if err != nil {
						t.Errorf("Some err happen when put. error = %e", err)
					}
				}
			}
			...
			if len(del.delList) != 0 {
				for _, key := range del.delList {
					ctx, cancel := context.WithTimeout(context.Background(), timeout)
					defer cancel()
					_, err = cli.Delete(ctx, tt.args.etcdPrefix+key)
					if err != nil {
						t.Errorf("Some err happen when del. error = %e", err)
					}
				}
			}
